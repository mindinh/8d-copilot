...
describe('buildEightDSystemPrompt', () => {
    it('không truyền gì thì ra đúng prompt mặc định', () => {
        ...
        // Câu đặc trưng của D4 mặc định
        expect(p).toContain('The most important discipline');
    ...
    it('ghi đè MỘT bước thì chỉ bước đó đổi', () => {
        ...
        expect(p).not.toContain('why this mix of skills suits');
    ...
    it('ghi đè bằng chuỗi rỗng hoặc khoảng trắng → rơi về mặc định', () => {
        ...
        for (const v of ['', '   ', '\n\n']) {
            expect(buildEightDSystemPrompt({ D4: v })).toContain('The most important discipline');
        }
    });
