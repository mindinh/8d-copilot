...
const D3_FORM_SCHEMA = JSON.stringify({ fields: [
    ...
    structuredField('containment.actions', 'Containment actions', 'action-cards', 'array', 12, { required: true, minItems: 1 }, { items: { type: 'object', properties: { action: { type: 'string', description: 'Short concise action description without boilerplate prefix.' }, status: { type: 'string', enum: ['Planned', 'In Process', 'Done', 'Verified'] } } } }),
...
const D4_FORM_SCHEMA = JSON.stringify({ fields: [
    structuredField('rootCause.statement', 'Root cause', 'ai-draft', 'string', 12, { required: true, minLength: 30, maxLength: 320 }),
    structuredField('rootCause.ishikawaBoard', 'Ishikawa', 'ishikawa-grid', 'array', 12, {}, { description: 'Recorded 6M assessment when the case carries one. When ishikawa is empty the AI proposes one entry per category, marked source=proposed.', items: { type: 'object', properties: { category: { type: 'string', enum: ['Man', 'Machine', 'Method', 'Material', 'Measurement', 'Environment'] }, finding: { type: 'string', maxLength: 220, description: 'The VERDICT for this branch, e.g. clamp pad worn 0.2 mm, or gauge R&R 8%, or not assessed. Never a list of evidence to gather - that goes in evidenceGaps once. Never the category name.' }, isRootCause: { type: 'boolean', description: 'True for at most ONE branch in the whole board.' }, source: { type: 'string', enum: ['recorded', 'proposed'] } } } }),
    structuredField('rootCause.fiveWhy', '5-Why chain', 'why-chain', 'array', 12, { required: true, minItems: 1 }, { items: { type: 'object', properties: { step: { type: 'integer', description: 'Position in the chain, starting at 1.' }, why: { type: 'string', maxLength: 140, description: 'The question only, ending in a question mark. No answer, no evidence, no citation.' }, answer: { type: 'string', maxLength: 200, description: 'The answer only. Never repeat the question, never list evidence still to be gathered.' }, evidence: { type: 'string', maxLength: 200, description: 'The CaseContext path or the measurement that backs the answer. Plain text, no markdown labels.' } } } }),
...
const D7_FORM_SCHEMA = JSON.stringify({ fields: [
    ...
    structuredField('preventive.actions', 'Preventive actions', 'action-cards', 'array', 12, { required: true, minItems: 1 }, { items: { type: 'object', properties: { action: { type: 'string', maxLength: 240, description: 'The action and its trigger. No owner, no status, no rationale - those are separate fields.' }, owner: { type: 'string', maxLength: 120, description: 'ONLY the person or function accountable. Leave empty when none is recorded.' }, status: { type: 'string', maxLength: 40, description: 'ONLY the current status, one or two words, e.g. "Planned", "In progress", "Done".' }, origin: { type: 'string', maxLength: 60, description: 'ONLY where it came from: "recorded" for an action already in the case, or "precedents#N" for a proposal.' }, protection: { type: 'string', maxLength: 200, description: 'One line.' } } } }),
    ...
    structuredField('sources', 'Evidence and traceability', 'evidence-list', 'array', 12, {}, { items: { type: 'string' } }),
], groups: [group('d7-ai-result', 'AI-generated preventive plan', ['preventive.objective', 'preventive.actions', 'preventive.fmea', 'sources'], '100', 10)] }, null, 2);
