...
import { normalizeFormSchema, parseConfig, stringifyConfig } from './step-prompt-editor/json';
...
const ALL_TABS: readonly StepEditorTab[] = ['schema', 'prompt', 'form', 'constraints'];
...
export function StepPromptEditorPage() {
    ...
    const [draft, setDraft] = useState<Record<ConfigField, string>>({ combinedPrompt: '', formSchemaJson: '', constraintsJson: '' });
    ...
    const loadDraft = useCallback((prompt: StepPrompt) => setDraft({
        combinedPrompt: prompt.combinedPrompt ?? prompt.systemPrompt ?? '',
    ...
    async function save() {
        if (!current || form.error || constraints.error) { toast.error('Fix invalid JSON before saving'); return; }
        setSaving(true);
        try {
