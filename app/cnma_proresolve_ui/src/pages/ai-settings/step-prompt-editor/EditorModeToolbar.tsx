...
export function EditorModeToolbar({ mode, onVisual, onJson, actions }: EditorModeToolbarProps) {
    return <div className="mb-4 flex shrink-0 items-center justify-between rounded-lg border bg-card p-2">
        <div className="flex gap-2">
            <Button size="sm" variant={mode === 'form' ? 'secondary' : 'ghost'} onClick={onVisual}><LayoutList className="h-4 w-4" /> Visual Editor</Button>
            <Button size="sm" variant={mode === 'json' ? 'secondary' : 'ghost'} onClick={onJson}><Code className="h-4 w-4" /> JSON Source</Button>
        </div>
        {actions}
    </div>;
}
