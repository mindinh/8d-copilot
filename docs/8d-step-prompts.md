...
|---|---|---|
| D1 — Establish the Team | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 390 |
| D2 — Describe the Problem | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 285 |
| D3 — Interim Containment Actions | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 129 |
| D4 — Root Cause Analysis | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 737 |
| D5 — Permanent Corrective Actions | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 169 |
| D6 — Verify Effectiveness | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 142 |
| D7 — Prevent Recurrence | `combinedPrompt` + `inputSchemaJson` + `formSchemaJson` | 191 |
...
```json
{
  ...
  "groups": [
    {
      "id": "d1-ai-result",
      "label": "AI-generated team recommendation",
      "fieldKeys": [
        "team.roster",
        "team.assignedRoster"
      ],
    ...
    "containment.actions": {
      ...
      "items": {
        ...
        "properties": {
          "action": {
            "type": "string",
            "description": "Short concise action description without boilerplate prefix."
          },
          "status": {
            "type": "string",
            "enum": [
...
## FIELD MECHANICS
Put the root cause conclusion in rootCause.statement and the recorded chain in rootCause.fiveWhy.
Expose evidence gaps separately in rootCause.evidenceGaps.
Treat precedent causes as hypotheses, not facts.
When ishikawa carries recorded entries, copy each one into rootCause.ishikawaBoard with source set to recorded. Do not reword a recorded finding.
When ishikawa is empty, propose one entry per 6M category from the evidence you do have, set source to proposed, and set finding to an empty string for a category you genuinely cannot speak to. Never let a proposal read as a recorded finding.
```
...
```json
{
  ...
  "properties": {
    ...
    "rootCause.ishikawaBoard": {
      "type": "array",
      "title": "Ishikawa",
      "description": "Recorded 6M assessment when the case carries one. When ishikawa is empty the AI proposes one entry per category, marked source=proposed.",
      "x-source": "ai_enrichment",
      "items": {
        "type": "object",
        "properties": {
          ...
          "finding": {
            "type": "string",
            "maxLength": 220,
            "description": "The VERDICT for this branch, e.g. clamp pad worn 0.2 mm, or gauge R&R 8%, or not assessed. Never a list of evidence to gather - that goes in evidenceGaps once. Never the category name."
          },
          "isRootCause": {
            "type": "boolean",
      ...
      "items": {
        ...
        "properties": {
          ...
          "why": {
            "type": "string",
            "maxLength": 140,
            "description": "The question only, ending in a question mark. No answer, no evidence, no citation."
          },
          "answer": {
            "type": "string",
            "maxLength": 200,
            "description": "The answer only. Never repeat the question, never list evidence still to be gathered."
          },
          "evidence": {
            "type": "string",
            "maxLength": 200,
            "description": "The CaseContext path or the measurement that backs the answer. Plain text, no markdown labels."
          }
        }
      }
    },
      ...
      "items": {
        ...
        "properties": {
          "action": {
            "type": "string",
            "maxLength": 240,
            "description": "The action and its trigger. No owner, no status, no rationale - those are separate fields."
          },
          "owner": {
            "type": "string",
            "maxLength": 120,
